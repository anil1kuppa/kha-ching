import { placeOrder } from "../../../kiteUtils"
...
const optionSellerSLWatcher = async ({
  ...
  try {
    ...
    const { order_id: reEntryOrderAckId } = await placeOrder(
      kite,
      kite.VARIETY_REGULAR,
      reEntryOrder
    )

    logger.info("reEntryOrderAckId", { reEntryOrderAckId })
  } catch (e) {
    logger.error("🔴 [optionSellerSLWatcher] error. Checker terminated!!", e)
